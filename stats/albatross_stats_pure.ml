...
let parse_proc_stat s =
  ...
  in
  pid :: tcomm :: Astring.String.cuts ~sep:" " rest
...
let linux_rusage pid =
  ...
  if List.length stat_vals >= 52 && List.length statm_vals >= 7 then
    ...
    (* i64 (List.nth stat_vals 21) >>= fun starttime -> *)
    i64 (List.nth stat_vals 22) >>= fun vsize -> (* in bytes *)
    i64 (List.nth stat_vals 31) >>= fun rss -> (* in pages *)
    ...
    and kmem = { Stats.vsize; rss; tsize; dsize; ssize; runtime = 0L; cow = 0; start = (0L, 0) }
    in
    Ok (rusage, kmem)
  else
